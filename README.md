...
## What I excluded from the public repo draft

I intentionally left out hidden R session artifacts such as `.RData` and `.Rhistory`, as well as third-party PDFs and textbook material that would clutter the repo or raise copyright questions. The user-authored reports and presentation files are preserved.

## Good next improvements before publishing

- Refactor repeated code into reusable helper functions.
- Add a single script that reproduces the headline metrics end to end.
- Save model outputs and plots into `results/` rather than relying on the interactive R workspace.
- Add a few cleaned figures to the README for faster recruiter scanning.
- Optionally translate the core workflow into Python or Quarto if you want broader industry familiarity.
