...
# Original source file: Design_nmlz0_1.R
# Purpose: Evaluate one 16-point design candidate against the full 256-point dataset.
...
x_train <- DataForRF[c(12,39,30,49,70,105,84,127,141,162,155,184,195,240,213,250) ,1:14]
y_train <- DataForRF[c(12,39,30,49,70,105,84,127,141,162,155,184,195,240,213,250) ,15]
x_test <- DataForRF[1:256, 1:14]
y_test <- DataForRF[1:256, 15]
...
cv.out =cv.glmnet (x_train_lasso, y_train, alpha = 1, nfold = 4)
plot(cv.out)
...
cv2.out =cv.glmnet (x_train_ridge, y_train, alpha = 0, nfold = 4)
plot(cv2.out)
